...
const login = () => {
  ...
  .then(response => {
    ...
    router.push({ name: 'Loginolva', id: response.data.user.id });
...
<template>
  <div class="">
    ...
    <div class="container bg-light rounded border border-5 border-white w-25" style="--bs-white-rgb: 139, 0, 0;">
      <form class="row g-3" @submit.prevent="login">
        <div class="col-md-12">
          ...
          <input v-model="email" required type="email" class="form-control" id="inputEmail" autocomplete="email"
            placeholder="Adja meg az email címét: pl kala.pal@gmail.com">
        </div>
        <br>
        <div class="col-md-12">
        ...
        <div class="col-12 p-3">
          <button type="submit" class="btn btn-dark darkred">Bejelentkezés</button>
        </div>
      </form>
